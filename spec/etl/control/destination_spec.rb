...
describe ETL::Control::Destination do
  ...
  context "When the records should be unique across field(s)" do
    before(:each) {
    ...
    context "When outputing only the records that are unique to a single field" do
...
DOCUMENT
    ...
    context "When outputing only the records that are unique across multiple fields" do
